...
class Compose(object):
    ...
    def __call__(self, image, label, boundary):
        for t in self.segtransform:
            image, label = t(image, label, boundary)
...
class ToTensor(object):
    ...
    def __call__(self, image, label, boundary):
        ...
        if not len(boundary.shape) == 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray boundary with 2 dims.\n"))
        ...
        if not isinstance(boundary, torch.LongTensor):
...
class Resize(object):
    ...
    def __call__(self, image, label, boundary):
        ...
        boundary = cv2.resize(boundary, self.size[::-1], interpolation=cv2.INTER_NEAREST)
        return image, label, boundary
...
class RandScale(object):
    ...
    def __call__(self, image, label, boundary):
        ...
        boundary = cv2.resize(boundary, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return image, label, boundary
...
class Crop(object):
    ...
    def __call__(self, image, label, boundary):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            boundary = cv2.copyMakeBorder(boundary, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half,
                                          cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        ...
        boundary = boundary[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w]
        return image, label, boundary


class RandRotate(object):
    # Randomly rotate image & label & boundary with rotate factor in [rotate_min, rotate_max]
    ...
    def __call__(self, image, label, boundary):
        if random.random() < self.p:
            ...
            boundary = cv2.warpAffine(boundary, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT,
                                      borderValue=self.ignore_label)
        return image, label, boundary
...
class RandomContrast(object):
    ...
    def __call__(self, image, label, boundary):
        ...
        image = np.uint8(np.clip((a * image + b * 255), 0, 255))
        return image, label, boundary
