...
def get_parser():
    parser = argparse.ArgumentParser(description='GDN')
    parser.add_argument('--config', type=str, default='config/cityscapes/cityscapes_gdn.yaml', help='config file')
    parser.add_argument('opts', help='see config/cityscapes/cityscapes_gdn.yaml for all options', default=None,
...
def main():
    ...
    if not args.has_prediction:
        if args.arch == 'gdn':
            ...
            model = GDN(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, pretrained=False)
        logger.info(model)
