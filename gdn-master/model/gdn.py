...
class GDN(nn.Module):
    def __init__(self, layers=12, bins=(1, 2, 3, 6), dropout=0.1, classes=19, zoom_factor=8, use_ppm=True,
    ...
    def forward(self, x, y=None):
        ...
        x = self.head(x)
        x1 = self.body_1(x)
        segment1 = self.seg1(x1)
        x = self.body_2(x1, x)
        # x = self.upsample(x)
        x2 = self.tail1(x)
        segment2 = self.seg2(x2)
        x = self.tail2(x2)
        if self.training:
            y1 = (F.interpolate(torch.unsqueeze(y.float(), dim=1), size=[64, 128], mode='nearest')).long()
            y1 = torch.squeeze(y1, dim=1)
            y2 = (F.interpolate(torch.unsqueeze(y.float(), dim=1), size=[256, 512], mode='nearest')).long()
...
class BASELINE(nn.Module):
    def __init__(self, pretrained=False):
        ...
        tail2 = [weight_norm(nn.Conv2d(108, 32, kernel_size=3, padding=1)),
                 ...
                 nn.Upsample(scale_factor=2, mode='nearest'),
                 nn.PReLU(),
                 weight_norm(nn.Conv2d(20, 19, kernel_size=3, padding=1)),
                 weight_norm(nn.Conv2d(19, 19, kernel_size=3, padding=1))]
        self.pre = Body_3()
        self.head = nn.Sequential(*head)
        self.body_1 = Body_1()
        self.body_2 = Body_2()
