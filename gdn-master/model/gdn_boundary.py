...
class GDN(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_ppm=True,
        ...
        waist = [weight_norm(nn.ConvTranspose2d(54, 54, stride=2, kernel_size=3, padding=1, output_padding=1)),
                 nn.PReLU(),
                 weight_norm(nn.ConvTranspose2d(54, 54, stride=2, kernel_size=3, padding=1, output_padding=1)),
                 nn.PReLU()]
        boundary = [weight_norm(nn.Conv2d(54, 54, kernel_size=3, padding=1)),
                    nn.PReLU(),
                    weight_norm(nn.Conv2d(54, 54, kernel_size=3, padding=1)),
                    nn.PReLU(),
                    weight_norm(nn.Conv2d(54, 36, kernel_size=3, padding=1)),
                    nn.PReLU(),
                    weight_norm(nn.Conv2d(36, 18, kernel_size=3, padding=1)),
                    nn.PReLU(),
                    weight_norm(nn.Conv2d(18, 18, kernel_size=3, padding=1)),
                    nn.PReLU(),
                    weight_norm(nn.Conv2d(18, 1, kernel_size=3, padding=1)),
                    weight_norm(nn.Conv2d(1, 1, kernel_size=3, padding=1))]
        tail = [weight_norm(nn.Conv2d(108, 32, kernel_size=3, padding=1)),
                nn.PReLU(),
        ...
        seg1 = [weight_norm(nn.Conv2d(54, 32, kernel_size=3, padding=1)),
                nn.PReLU(),
                weight_norm(nn.Conv2d(32, 20, kernel_size=3, padding=1)),
                nn.PReLU(),
                weight_norm(nn.Conv2d(20, classes, kernel_size=3, padding=1)),
                weight_norm(nn.Conv2d(classes, classes, kernel_size=3, padding=1))]
        seg2 = [weight_norm(nn.Conv2d(108, 32, kernel_size=3, padding=1)),
                nn.PReLU(),
                weight_norm(nn.Conv2d(32, 20, kernel_size=3, padding=1)),
                nn.PReLU(),
                weight_norm(nn.Conv2d(20, classes, kernel_size=3, padding=1)),
    ...
    def forward(self, x, y=None, z=None):
        ...
        x = self.head(x)
        wai = self.waist(x)
        boundary = self.boundary(wai)
        body1 = self.body_1(x)
        segment1 = self.seg1(body1)
        body2 = self.body_2(body1)
        ff = self.ffm(wai, body2)
        segment2 = self.seg2(ff)
        x = self.tail(ff)
        if self.training:
            y1 = (F.interpolate(torch.unsqueeze(y.float(), dim=1), size=[90, 120], mode='nearest')).long()
            y1 = torch.squeeze(y1, dim=1)
            y2 = (F.interpolate(torch.unsqueeze(y.float(), dim=1), size=[360, 480], mode='nearest')).long()
            ...
            aux_loss = 1 - ssim(img_sr, img) + 0.25 * self.boundary_loss_function(boundary,
                                                                                  z1)  # If there is no GT of boundary information,it is recommended not to add boundary loss
            f = open('loss.txt', 'a')
